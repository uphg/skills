...
## 2026-09-07 — 函数命名重构为分组列表

将扁平的函数速查表 + 独立消歧规则列表,替换为单个嵌套无序列表。`to` / `from` / `as` 收拢到一个"类型转换"父级条目下,分组关系一目了然;其余各模式将各自的消歧规则内联到条目中,消除了表格与规则列表的内容重复。仅调整信息架构——所有规则、示例与禁令均保留。

### 变更清单

### 1. `SKILL.md` → Functions 小节重构

- 将 `场景 | 模式 | 示例` 速查表与四条消歧规则替换为一个嵌套无序列表。
- `to` / `from` / `as` 收拢到单个**类型转换**父级条目下,设三个子条目,每条携带模式 + 返回值类型语义 + 示例;`parse`/`convert` 禁令移入父级条目。
- 其余模式(事件处理器、Async、api、read/write、get/set)按原顺序改为同级条目;各自的消歧规则(Async 语义、api 永不加 Async、存储位置判别)内联进对应条目。
